...
# ============================================
$mutexName = "Global\CNMTray_SingleInstance"
$script:mutex = $null
$script:createdNew = $false

try {
    $script:mutex = [System.Threading.Mutex]::new($true, $mutexName, [ref]$script:createdNew)
} catch {
    $script:createdNew = $false
}
...
function Start-TrayApp {
    ...
    $startItem.Add_Click({
        ...
        if (Start-CNMServer) {
            $notifyIcon.ShowBalloonTip(2000, "Service Started", "CNM relay server running on port 3001", [System.Windows.Forms.ToolTipIcon]::Info)
        } else {
            $notifyIcon.ShowBalloonTip(2000, "Celio's Network Machine", "Failed to start server", [System.Windows.Forms.ToolTipIcon]::Error)
        }
        Update-MenuState
    })
    ...
    $stopItem.Add_Click({
        ...
        $notifyIcon.ShowBalloonTip(2000, "Service Stopped", "CNM relay server stopped", [System.Windows.Forms.ToolTipIcon]::Info)
        Update-MenuState
    })
    ...
    if (Start-CNMServer) {
        $notifyIcon.ShowBalloonTip(3000, "Service Started", "CNM relay server running on port 3001", [System.Windows.Forms.ToolTipIcon]::Info)
    } else {
        $notifyIcon.ShowBalloonTip(3000, "Celio's Network Machine", "Server may already be running or failed to start", [System.Windows.Forms.ToolTipIcon]::Warning)
    }
